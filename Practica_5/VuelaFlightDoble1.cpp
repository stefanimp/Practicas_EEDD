...
VuelaFlightDoble1::VuelaFlightDoble1(): airports(THashAeropuerto(10)) {
 ...
 * */
VuelaFlightDoble1::VuelaFlightDoble1(std::ifstream &fichAirports, std::ifstream &fichAerolines, std::ifstream &fichRoutes, std::ifstream &fichFlights, unsigned long int maxAirpotsToInsert, float lambda): airports(THashAeropuerto(maxAirpotsToInsert, lambda)) {
    std::string fila;
 ...
 * */
void VuelaFlightDoble1::addAeropuerto(const Aeropuerto &aeropuerto) {
    unsigned long  clave = airports.funcionHashing(aeropuerto.getIata());
    airports.insertarDoble1(clave, aeropuerto.getIata(), aeropuerto);
}
 ...
 * */
Ruta* VuelaFlightDoble1::buscarRutasOriDes(const std::string &idAerOrig, const std::string &idAerDest) {
 ...
 * */
std::list<Ruta*> VuelaFlightDoble1::buscarRutasOrigen(const std::string &idAerOrig) {
 ...
 * */
std::list<Aeropuerto*> VuelaFlightDoble1::buscarAeropuertoPais(const std::string &pais) {
 ...
 * */
Ruta* VuelaFlightDoble1::addNewRute(const std::string &idAerOrig, const std::string &idAerDest, const std::string &aerolin) {
    ...
    if(!airports.buscarDoble1(idAerOrig)){
        return nullptr;
    }
    rutanueva.setOrigin(*airports.buscarDoble1(idAerOrig));

    aeropComprobacion.setIata(idAerDest);
    if(!airports.buscarDoble1(idAerDest)){
        return nullptr;
    }
    std::multimap<std::string, Ruta*>::iterator iteratorRoutesDest = routesDest.begin();
    Ruta *rutaP = iteratorRoutesDest->second;
    rutanueva.setDestination(*airports.buscarDoble1(idAerDest));
...
Aerolinea *VuelaFlightDoble1::buscaAerolinea(const std::string &icaoAerolinea) {
    Aerolinea *aerolinea;
...
std::list<Aerolinea *> VuelaFlightDoble1::buscarAerolineasActivas() {
...
std::list<Aerolinea *> VuelaFlightDoble1::getAerolineasPais(const std::string &idPais) {
    std::list<Aerolinea*> listAerolines;
    for (std::map<std::string, Aerolinea>::iterator iteradorAerlines = aerolines.begin(); iteradorAerlines != aerolines.end() ; ++iteradorAerlines) {
...
bool VuelaFlightDoble1::registrarVuelo(const std::string flightNum, const std::string iataAeroOrig, const std::string iataAeroDest,
    ...
    if(!airports.buscarDoble1(iataAeroOrig)){
        return false;
    }

    if(!airports.buscarDoble1(iataAeroDest)){
        return false;
    }
    ...
    newFlight.setAirportOrig(*airports.buscarDoble1(iataAeroOrig));
    newFlight.setAirportDest(*airports.buscarDoble1(iataAeroDest));
...
std::list<Vuelo *> VuelaFlightDoble1::buscaVuelos(const std::string &fNumber) {
...
std::list<Vuelo*> VuelaFlightDoble1::vuelosOperadosPor(const std::string &icaoAerolinea, const Fecha &fecha) {
...
std::set<std::string> VuelaFlightDoble1::buscaVuelosDestAerop(const std::string &paisOrig, const std::string &iataAeropDest) {
...
std::set<Aeropuerto *> VuelaFlightDoble1::buscaAeropuertosAerolinea(const std::string &icaoAerolinea) {
    Aerolinea *aerolinea;
...
Aeropuerto *VuelaFlightDoble1::buscaAeropuertoIata(const std::string &iata) {
    return airports.buscarDoble1(iata);
}

unsigned int VuelaFlightDoble1::getNumAeropuertos() const {
    return airports.getnumElementos();
}

std::list<Aeropuerto *> VuelaFlightDoble1::getAeropuertos() {
...
void VuelaFlightDoble1::eliminarAirport(const std::string &iata) {
    ...
    airports.borrarDoble1(iata);

}

//TODO comprobar este metodo
void VuelaFlightDoble1::eliminarAirportsInactivos() {
    for (int i = 0; i < airports.getTamTabla(); ++i) {
        Aeropuerto *airport = &airports[i];
        if(airport){
            if (airport->getTipo() == "closed"){
                airports.borrarDoble1(airport->getIata());
            }
        }
    }
}

void VuelaFlightDoble1::mostrarEstadoTabla() const {
    std::cout<<"====================================================" <<std::endl;
