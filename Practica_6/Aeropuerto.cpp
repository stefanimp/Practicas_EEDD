 ...
 * */
Aeropuerto::Aeropuerto(const Aeropuerto &orig): id(orig.id), iata(orig.iata), tipo(orig.tipo), nombre(orig.nombre), continente(orig.continente), isoPais(orig.isoPais),posicion(orig.posicion) {
...
float Aeropuerto::getX() const {
    return posicion.getLongitud();
}

float Aeropuerto::getY() const {
